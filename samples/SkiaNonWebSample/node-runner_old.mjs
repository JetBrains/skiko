...
const nativeGLAddonPath = process.env.SKIKO_NATIVE_GL_ADDON;
...
class CpuCopyRenderer {
    ...
    createCanvasContext(canvas, attributes) {
        console.log(`[CPU-COPY] Creating web gl context for ${canvas.id}`);
...
class NativeWindowRenderer {
    constructor(nativeWindow) {
        ...
        console.log("[NATIVE] Creating web gl rendering context");
    ...
    createCanvasContext(canvas, attributes) {
        console.log(`[NATIVE] Creating web gl context for ${canvas.id}`);
        const gl = nodeGles.createWebGLRenderingContext({
            width: canvas.width,
            height: canvas.height,
            majorVersion: 3,
...
async function runWindowedDemo() {
    ...
    if (nativeGLAddonPath) {
        globalThis.__skikoNativeHost = require("./native-skiko/build/Release/native_skiko.node");
        globalThis.__skikoLogGLFallbacks = process.env.SKIKO_LOG_GL_FALLBACKS === "1";
    }

    if (!useCpuCopy && false) {
        console.log("Using native OpenGL presentation.");
