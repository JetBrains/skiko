...
object SkikoProperties {
    ...
    /**
     * Is ANGLE render API enabled (https://skia.org/docs/user/special/angle/).
     *
    ...
    internal fun fallbackRenderApiQueue(initialApi: GraphicsApi) : List<GraphicsApi> {
        var fallbackApis = when (hostOs) {
            ...
            OS.Windows -> when (hostArch) {
                // Skia isn't properly tested on OpenGL and Windows ARM (https://groups.google.com/g/skia-discuss/c/McoclAhLpvg?pli=1)
                Arch.Arm64 -> listOfNotNull(
                    if (renderingAngleEnabled) GraphicsApi.ANGLE else null,
                    GraphicsApi.DIRECT3D, GraphicsApi.SOFTWARE_FAST, GraphicsApi.SOFTWARE_COMPAT)
                else -> listOfNotNull(
                    if (renderingAngleEnabled) GraphicsApi.ANGLE else null,
                    GraphicsApi.DIRECT3D, GraphicsApi.OPENGL, GraphicsApi.SOFTWARE_FAST, GraphicsApi.SOFTWARE_COMPAT)
            }
