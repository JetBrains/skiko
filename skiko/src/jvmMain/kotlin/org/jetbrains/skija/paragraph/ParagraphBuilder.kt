...
class ParagraphBuilder(style: ParagraphStyle?, fc: FontCollection?) :
    Managed(_nMake(Native.Companion.getPtr(style), Native.Companion.getPtr(fc)), _FinalizerHolder.PTR) {
    companion object {
    ...
    fun addPlaceholder(style: PlaceholderStyle): ParagraphBuilder {
        ...
        _nAddPlaceholder(
            ...
            style.getWidth(),
            style.getHeight(),
            style.getAlignment().ordinal(),
            style.getBaselineMode().ordinal(),
            style.getBaseline()
        )
        return this
    }
