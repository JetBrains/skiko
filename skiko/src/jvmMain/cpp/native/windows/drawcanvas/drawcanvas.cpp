...
JNIEXPORT void JNICALL Java_org_jetbrains_awthrl_Components_Window_updateLayer(JNIEnv *env, jobject window)
{
	...
	if (dsi != NULL)
	{
		...
		int  iPixelFormat;
		iPixelFormat = ChoosePixelFormat(wndHandler, &pixFormatDscr);
		...
		if (windowRef == 0) {
			fprintf(stderr, "Cannot create global reference!\n");
		}
...
JNIEXPORT jfloat JNICALL Java_org_jetbrains_awthrl_Components_Window_getContentScale(JNIEnv *env, jobject window) {
	...
	if (layer != NULL) {
		...
		float result = getScaleFloatFromEnum(scaleFactor);
		fprintf(stderr, "scale factor %f", result);
		return result;
	}
	return 1.0f;
}