 ...
 *
 * This [MetalRenderer] draws content on-screen for maximum efficiency,
...
internal class MetalRenderer(
...
) : ContextBasedRenderer(layer, analytics, GraphicsApi.METAL, "Metal") {
    companion object {
        init {
            Library.load()
        }
    ...
    /**
     * [MetalDevice] initialized for given [layer] or null if [MetalRenderer] is disposed,
    ...
    override fun syncBoundsFromPlatformComponent() = synchronized(drawLock) {
        ...
        Logger.debug { "MetalRenderer#resizeLayers $this {x: $x y: $y width: $width height: $height} rootPane: ${rootPane.size}" }
        resizeLayers(device.ptr, x, y, width, height)
    }

    override fun setVisible(isVisible: Boolean) {
        Logger.debug { "MetalRenderer#setVisible($isVisible)" }
    ...
    // After `flush` you also need to call `finishFrameAsync` (or `finishFrameSync`).
    override fun flush() {
        super.flush()
     ...
     */
    private fun finishFrameAsync() = finishFrameAsync(device.ptr)
     ...
     */
    private fun finishFrameSync() = finishFrameSync(device.ptr)
