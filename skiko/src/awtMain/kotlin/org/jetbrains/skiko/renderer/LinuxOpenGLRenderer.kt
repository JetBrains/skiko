...
internal class LinuxOpenGLRenderer(
    ...
    companion object {
        ...
        private val frameDispatcher = FrameDispatcher(MainUIDispatcher) {
            ...
            try {
                for (renderer in toRedrawVisible) {
                    drawingSurfaces[renderer]!!.makeCurrent(renderer.glContext)
                    renderer.drawFrame()
                }
                ...
                val vsyncRenderer = toRedrawVisible
                    .filter { it.properties.isVsyncEnabled }
                    .maxByOrNull { it.frameLimit }

                for (renderer in toRedrawVisible.filter { it != vsyncRenderer }) {
                    drawingSurfaces[renderer]!!.makeCurrent(renderer.glContext)
                ...
                if (vsyncRenderer != null) {
                    drawingSurfaces[vsyncRenderer]!!.makeCurrent(vsyncRenderer.glContext)
                    drawingSurfaces[vsyncRenderer]!!.setSwapInterval(1)
                    drawingSurfaces[vsyncRenderer]!!.swapBuffers()
                    OpenGLApi.instance.glFlush()
                }
            } finally {
