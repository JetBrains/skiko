 ...
 */
internal class SkikoFramePacingService internal constructor(
    ...
    override fun refreshPeriodNanos(displayId: Long): Long =
        findDevice(displayId)?.let(::refreshPeriodNanos) ?: 0

    private fun refreshPeriodNanos(device: GraphicsDevice): Long {
    ...
    override fun subscribe(
        ...
        val device = findDevice(displayId) ?: return null
        val periodNanos = refreshPeriodNanos(device)

        // A clock is shared only while it can still serve a new subscriber. One whose thread has
        // ended never ticks again, and one made for another refresh rate paces to the old mode; either
        // stays with the subscribers it has until they leave, and a new subscriber gets a new clock.
        val shared = clocks[displayId]?.takeUnless { it.ended || it.periodNanos != periodNanos }
        val clock = shared ?: run {
            val created = newClock(displayId, periodNanos) ?: return null
...
internal abstract class DisplayClock(
    ...
    /**
     * Set by the clock's thread when it ends, which it also does on its own: a source that fails or
     * cannot be opened. Such a clock never ticks again.
     */
    @Volatile
    var ended = false
        protected set
