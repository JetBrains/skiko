...
// awtMain/objectiveC/macos/MetalRenderer.mm's `extern JavaVM *jvm;`.
extern "C" JavaVM *jvm;
 ...
 * swap() calls that happen as part of rendering a frame (see Java_..._Direct3DRenderer_swap in
 * directXRenderer.cc). VsyncTicker is a *separate*, render-path-independent tick source: one
...
DWORD WINAPI vsyncThreadProc(LPVOID param) {
    ...
    // the class that is actually loaded, the same way MetalRenderer.mm's
