...
class LibraryLoadCleanupTest {
    ...
    fun `load library - purges old content in skiko data dir`() = runTest {
        ...
        /*
        Prepare data dir to contain old and recent directories
         */
        val oldDirectory = tempDataDir.resolve("old").createDirectories()
        val newDirectory = tempDataDir.resolve("new").createDirectories()
        ...
        oldDirectory.setLastModifiedTime(oldDirectoryTime)
        newDirectory.setLastModifiedTime(newDirectoryTime)
        ...
        assertEquals(newDirectoryTime, newDirectory.getLastModifiedTime())
        ...
        tempDataDir.listDirectoryEntries().forEach { file ->
            when {
                file.isReadable() && file.name == ".lock" -> return@forEach
