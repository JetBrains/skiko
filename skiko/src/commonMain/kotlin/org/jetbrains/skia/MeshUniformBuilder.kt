...
class MeshUniformBuilder internal constructor(ptr: NativePointer) : Managed(ptr, _FinalizerHolder.PTR) {
     ...
     *                                  declares it with a different size
     */
    fun uniform(name: String, value: IntArray) {
        ...
        checkStatus(status, name, value.size)
    }
    ...
    private fun checkStatus(status: Int, name: String, valueSize: Int) {
        when (status) {
            STATUS_OK -> Unit
            STATUS_UNKNOWN_UNIFORM ->
                ...
                throw IllegalArgumentException(
                    "Uniform '$name' is not $valueSize values wide"
                )
        }
    }
