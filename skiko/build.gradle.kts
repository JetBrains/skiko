    ...
    if (supportNative) {
        val nativeTarget = when (target) {
            "macos-x64", "macos-arm64" -> macosX64() {
                compilations.all {
                    kotlinOptions {
                        freeCompilerArgs += listOf("-include-binary", "$buildDir/nativeBridges/static/$target/skiko-native-bridges-$target.a")
                    }
                }
            }
            else -> null
...
// Very hacky way to compilenative bridges and add the
...
project.tasks.register<Exec>("nativeBridgesCompile") {
    ...
    val srcs = inputDirs
        ...
        .map { it.absolutePath }
        .toTypedArray()
    val outs = srcs.map { it.substringAfterLast("/") }.map { File(it).nameWithoutExtension }
    workingDir = File(outDir)
    commandLine = listOf(
        "clang++",
        *targetArch.clangFlags,
...
project.tasks.register<Exec>("nativeBridgesLink") {
    ...
    val inputDirs = listOf("$buildDir/nativeBridges/obj/$target")
    ...
    val srcs = inputDirs
        ...
        .map { it.absolutePath }
        .toTypedArray()
    val outs = arrayOf("$outDir/skiko-native-bridges-$target.a")
    workingDir = File(outDir)
    commandLine = listOf(
