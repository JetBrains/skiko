...
abstract class BuildLocalSkiaTask : DefaultTask() {
    ...
    fun buildSkia() {
        ...
        val toolsDir = File(skiaPackRoot, "tools/skia_release")
        if (!toolsDir.isDirectory) {
            throw GradleException(
                "Directory tools/skia_release not found in ${skiaPackRoot.absolutePath}\n" +
                "Expected: skia-pack repository root containing tools/skia_release/\n" +
    ...
    private fun runPythonScript(skiaPackRoot: File, script: String, vararg args: String) {
        ...
        if (!scriptFile.exists()) {
            throw GradleException(
                ...
                "Expected: skia-pack directory with tools/skia_release/$script\n" +
