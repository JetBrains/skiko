...
import WasmProposalsManifestTask
...
fun SkikoProjectContext.declareWasmTasks() {
    ...
    if (!isSideModule) {
        val buildWasmProposalsManifest = project.tasks.register("buildWasmProposalsManifest", WasmProposalsManifestTask::class.java) {
            group = "wasm validation"
            description = "Builds the WebAssembly proposals manifest from the optimized .wasm binary."
            dependsOn(optimizeWasm)
            optimizer.set(optimizeWasm.flatMap { it.optimizer })
            wasmFile.convention(
        ...
        project.tasks.register("dumpWasmProposalsManifest") {
            group = "wasm validation"
            description = "Copies the built wasm proposals manifest to the project as the reference file."
            dependsOn(buildWasmProposalsManifest)
            val builtManifest = buildWasmProposalsManifest.flatMap { it.manifestFile }
        ...
        project.tasks.register("verifyWasmProposalsManifest") {
            group = "wasm validation"
            description = "Verifies that the built wasm proposals manifest matches the checked-in reference."
            dependsOn(buildWasmProposalsManifest)
            val builtManifest = buildWasmProposalsManifest.flatMap { it.manifestFile }
            inputs.file(builtManifest)
